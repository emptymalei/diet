...
def count_column_values_within_ranges(df_inp, column_name, bins = None ):
    ...
    couting_data = {'price': bins[:-1], 'count': df_counting['count'].values, 
...
def count_column_values_within_ranges_two_levels_deep(
    df_inp, first_groupby_column_name, 
    ...
    for first_groups_key, one_df_of_first_groups in df_first_groups:
        ...
        df_second_level_groups = one_df_of_first_groups.groupby(second_groupby_column_name)
        for second_groups_key, one_df_of_second_groups in df_second_level_groups:
            counting_data_of_one_group[second_groups_key] = count_column_values_within_ranges(one_df_of_second_groups, 
                                                                                              count_column_name, 
                                                                                              bins)
